...
.nav-item {
  ...
  .children {
    ...
    > .children-nav-item > .body, > .children-nav-item > .children > .children-nav-item {
      width: 100%;
      height: auto;
      float: left;
      color: #333333;
        ...
        <div class="nav-item" v-for="nav in datas" :key="nav.id">
          ...
          <div class="children">
            <div class="children-nav-item" v-for="childrenNav in nav.children" :key="childrenNav.id">
              ...
              <div class="children">
                <div class="children-nav-item" v-for="finalChild in childrenNav.children" :key="finalChild.id">
                  <div class="name">{{ finalChild.name }}</div>
                  <div class="option">
                    <p-del-button permission="nav.destroy" @click="remove(finalChild)"></p-del-button>
                    <p-button glass="h-btn h-btn-s h-btn-primary" permission="nav.edit" text="编辑" @click="edit(finalChild)"></p-button>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    getData(reload = false) {
      ...
      this.loading = true;
      R.SpecialProject.List(this.pagination).then(resp => {
    ...
    remove(item) {
      R.SpecialProject.Delete({ id: item.id }).then(resp => {
